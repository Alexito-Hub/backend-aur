...
function post(p: string, fields: Record<string, string>, file?: { name: string; buffer: Buffer; mime: string }): Promise<string> {
...
export async function deepai(query: string, imagePath?: string): Promise<{ ai_msg: string }> {
    ...
    if (imagePath) {
        ...
        const up   = await post("/chat_attachments/upload", {}, { name, buffer: buf, mime: "image/jpeg" });
    ...
    await post("/save_chat_session", { uuid: session, title: "", chat_style: "chat", messages: history });
    ...
    return { ai_msg: await post("/hacking_is_a_serious_crime", fields) };
}
