...
const TOKEN   = COOKIES.find(c => c.name === "kimi-auth")!.value;
const COOKIE_STR = COOKIES.map(c => `${c.name}=${c.value}`).join("; ");

function baseHeaders(opts: KimiOptions = {}): Record<string, string> {
    return {
...
function decodeFrames(buf: Buffer): any[] {
    const out: any[] = [];
    let offset = 0;
...
function post(p: string, body: Buffer, headers: Record<string, any>): Promise<{ status: number; headers: Record<string, any>; body: Buffer }> {
    return new Promise((resolve, reject) => {
        const req = https.request({ hostname: "www.kimi.com", port: 443, path: p, method: "POST", headers: { ...headers, "Content-Length": String(body.length) } }, res => {
            ...
            res.on("end", async () => {
                try { resolve({ status: res.statusCode!, headers: res.headers, body: await decompress(Buffer.concat(c), res.headers["content-encoding"] as string) }); }
                catch (e) { reject(e); }
            });
        });
...
export async function kimiai(query: string, chatId: string | null = null, opts: KimiOptions = {}): Promise<KimiResult> {
    const payload: any = {
    ...
    const res = await post("/apiv2/kimi.gateway.chat.v1.ChatService/Chat", encodeFrame(payload), { ...baseHeaders(opts), "Content-Type": "application/connect+json", "Accept": "*/*" });
    if (res.status !== 200) throw new Error(`HTTP ${res.status}: ${res.body.toString()}`);

    const frames = decodeFrames(res.body);
...
export async function listChats(pageSize = 5, query = "", opts: KimiOptions = {}): Promise<any> {
    const body = Buffer.from(JSON.stringify({ project_id: "", page_size: pageSize, query }), "utf8");
    const res  = await post("/apiv2/kimi.chat.v1.ChatService/ListChats", body, { ...baseHeaders(opts), "Content-Type": "application/json", "Accept": "*/*" });
    if (res.status !== 200) throw new Error(`HTTP ${res.status}: ${res.body.toString()}`);
